namespace TrotiNet
{
    ...
    public abstract class AbstractProxyLogic
    {
        ...
        /// </summary>
        protected HttpSocket SocketBPClient;

        /// <summary>
        /// Socket dedicated to the proxy-server (remote) communication
        /// </summary>
        protected HttpSocket SocketPSRemote;

        public IPAddress InterfaceToBind;
        ...
        protected void Connect(string hostname, int port)
        {
            ...
            if (DestinationHostName != null &&
                ...
                (SocketPSRemote != null && !SocketPSRemote.IsSocketDead()))
                // Nothing to do, just reuse the socket
                return;

            if (SocketPSRemote != null)
            {
            ...
            foreach (var ip in ips)
            {
                try
                {
                    socket = new Socket(ip.AddressFamily, SocketType.Stream,
                        ProtocolType.Tcp);
                    socket.Bind(new IPEndPoint(InterfaceToBind, 0));
            ...
            SocketPSRemote = new HttpSocket(socket);
        ...
        protected void TunnelBP(byte[] msg, uint position, uint to_send)
        {
            if (to_send == 0)
                return;
            if (SocketBPClient.WriteBinary(msg, position, to_send) < to_send)
                throw new IoBroken();
        }
        ...
        protected void TunnelPS(byte[] msg, uint position, uint to_send)
        {
            if (to_send == 0)
                return;
            if (SocketPSRemote.WriteBinary(msg, position, to_send) < to_send)
                throw new IoBroken();
        }
    }
    ...
    public class BaseProxyLogic: AbstractProxyLogic
    {
        ...
        protected virtual void ReadRequest()
        {
            try
            {
                RequestLine = new HttpRequestLine(SocketBPClient);
            }
            ...
            RequestHeaders = new HttpHeaders(SocketBPClient);
        ...
        private void HandleWebSocket()
        {
            ...
            var socketsToConnect = new[] { this.SocketPSRemote, this.SocketBPClient };
        ...
        protected virtual void SendRequest()
        {
            ...
            RequestLine.SendTo(SocketPSRemote);
            RequestHeaders.SendTo(SocketPSRemote);
            if (State.bRequestHasMessage)
            {
                // Tunnel the request message
                if (State.bRequestMessageChunked)
                    SocketBPClient.TunnelChunkedDataTo(SocketPSRemote);
                else
                {
                    System.Diagnostics.Debug.Assert(
                        State.RequestMessageLength > 0);
                    SocketBPClient.TunnelDataTo(TunnelPS, State.RequestMessageLength);
                }
            }
        ...
        protected virtual void ReadResponse()
        {
            ...
            ResponseStatusLine = new HttpStatusLine(SocketPSRemote);
            ResponseHeaders = new HttpHeaders(SocketPSRemote);
    ...
    public class ProxyLogic: BaseProxyLogic
    {
        ...
        byte[] GetChunkedContent()
        {
            ...
            while (true)
            {
                string chunk_header = SocketPSRemote.ReadAsciiLine();
                ...
                SocketPSRemote.TunnelDataTo(buffer, size);
        ...
        byte[] GetNonChunkedContent()
        {
            ...
            SocketPSRemote.TunnelDataTo(buffer, ResponseMessageLength);
